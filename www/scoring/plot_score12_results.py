...
parser = argparse.ArgumentParser(
    description="Get correlations and plot Rosetta score12 predictions with experimental binding affinities"
)
parser.add_argument(
    "-in",
    help="data table output by build_models_score12.py",
    type=str,
    dest="infile",
    required=True,
)
...
def main():
    ...
    plt.legend(
        tuple(my_handles), tuple(my_labels), ncol=2, loc="upper center", numpoints=1
    )
    plt.title("ATLAS TCR-pMHC complexes")
    plt.xlabel("Experimentally measured binding affinity (kcal/mol)")

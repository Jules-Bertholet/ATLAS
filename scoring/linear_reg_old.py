...
parser = argparse.ArgumentParser(
    description="Perform multilinear regression on ATLAS data with Rosetta energy terms"
)
parser.add_argument(
    "-in",
    help="data table output by build_models.py",
    type=str,
    dest="infile",
    required=True,
)
...
def gibbs_free_energy(Kd):
    ...
    """
    dG = 0.001987 * 298.15 * np.log(Kd * 10.0**-6)
    return dG
...
def load_data(fh, start_col, num_features):
    ...
    for line in fh:
        ...
        # Design matrix
        X.append(list(map(float, split_line[start_col : start_col + num_features])))
...
def main():
    ...
    plt.plot(list(range(-15, 1)), list(range(-15, 1)), c="r")
    plt.xlim(-13, -4)
    plt.ylim(-12, -3)

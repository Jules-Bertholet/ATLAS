...
def calculate_chi(my_pose, res_num):
    ...
    testy = rosetta.core.pack.dunbrack.rotamer_from_chi(
        my_pose.residue(res_num), new_vec
    )
    print(my_pose.residue(res_num).name())
    chi_angles = my_pose.residue(res_num).chi()
    print(chi_angles)
    rotamer_states = list(map(int, list(new_vec)))
    print(list(map(int, list(new_vec))))
...
def main():
    ...
    OUT.write(",".join(map("{:.2f}".format, list(map(float, list(temp_chis))))) + "\t")
    OUT.write(",".join(map("{:.2f}".format, list(map(float, list(true_chis))))) + "\n")

...
parser = argparse.ArgumentParser(
    description="build model complexes using Rosetta's backrub app"
)
parser.add_argument(
    "-f",
...
def get_pivot_residues(pdb_file, cdr_seqs):
    ...
    for cdr in row.values[0]:
        ...
        pivot_positions = pivot_positions + list(
            range(srch_obj.start(), srch_obj.end())
        )
...
def backrub(pdb, label, pivot_residues):
    ...
    backrub_cmd = (
        [
            ...
            "-e",
            label + ".err",
            os.path.join(args.ros_path, "source/bin/backrub.static.linuxgccrelease"),
            "-database",

...
open( PDBOUT, "> $trunc_pdb" )
  || die("unable to open output pdb file: $trunc_pdb\n");
...
foreach my $line (@pdb_lines) {
    if (   ( substr( $line, 0, 4 ) eq "ATOM" )
        && ( substr( $line, 21, 1 ) eq $mhc_chain ) )
    {
        if ( substr( $line, 22, 4 ) eq " 181" ) {
            last;
        }    # truncate at residue 180
        elsif ( ( $classII == 1 ) && ( substr( $line, 22, 4 ) eq "  84" ) ) {
            last;
        }    # truncate at residue 83
        my $line2 = $line;
        substr( $line2, 21, 1 ) = "A";
...
if ( $classII == 1 ) {
    ...
    foreach my $line (@pdb_lines) {
        if (   ( substr( $line, 0, 4 ) eq "ATOM" )
            && ( substr( $line, 21, 1 ) eq $mhc_b_chain ) )
        {
            if ( substr( $line, 22, 4 ) eq "  94" ) {
                last;
            }    # truncate at residue 93
            my $line2 = $line;
            substr( $line2, 21, 1 ) = "B";
...
foreach my $line (@pdb_lines) {
    if (   ( substr( $line, 0, 4 ) eq "ATOM" )
        && ( substr( $line, 21, 1 ) eq $tcra_chain ) )
    {
        my $line2 = $line;
        substr( $line2, 21, 1 ) = "D";
        print TMP $line2;
...
foreach my $line (@pdb_lines) {
    if (   ( substr( $line, 0, 4 ) eq "ATOM" )
        && ( substr( $line, 21, 1 ) eq $tcrb_chain ) )
    {
        my $line2 = $line;
        substr( $line2, 21, 1 ) = "E";
        print TMP2 $line2;
...
foreach my $line (@mhc_b_lines) {
    print PDBOUT $line;
}    # in case this is ClassII
...
foreach my $line (@tcra_lines) {
    ...
    elsif ( ( $last_res_found == 1 ) && ( $orig_res ne $last_alpha_res ) ) {
        last;
    }
...
foreach my $line (@tcrb_lines) {
    ...
    elsif ( ( $last_res_found == 1 ) && ( $orig_res ne $last_beta_res ) ) {
        last;
    }

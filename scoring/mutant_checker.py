    ...
    for i in range(len(muts)):
        ...
        for line in PDB_f:
            ...
            if chain_str:
                if (
                    splitline[0] == "ATOM"
                    and splitline[4] == chains[i]
                    and splitline[5] == muts[i][1:-1]
                ):
                    found_pos = True
                    if aa_map[muts[i][0]] == splitline[3]:
                        ...
                        print(
                            "ERROR: template residue does not match literature residue"
                        )
                        return 0
            else:
                if (
    ...
    for i, row in df.iterrows():
        if pd.notnull(row["template_PDB"]):
            ...
            if row["TCR_mut"].strip() != "WT":
                result = mutant_check(
                    str(row["TCR_mut"]).strip(),
                    str(row["template_PDB"]).strip(),
                    p,
                    str(row["TCR_PDB_chain"]).strip(),
                )
                if not result:
                    print(
                        "Index: " + str(row["Index"]),
            ...
            if row["MHC_mut"].strip() != "WT":
                result = mutant_check(
                    str(row["MHC_mut"]).strip(),
                    str(row["template_PDB"]).strip(),
                    p,
                    str(row["MHC_mut_chain"]).strip(),
                )
                if not result:
                    print(
                        "Index: " + str(row["Index"]),
            ...
            if row["PEP_mut"].strip() != "WT":
                result = mutant_check(
                    str(row["PEP_mut"]).strip(), str(row["template_PDB"]).strip(), p
                )
                if not result:
                    print(
                        "Index: " + str(row["Index"]),
